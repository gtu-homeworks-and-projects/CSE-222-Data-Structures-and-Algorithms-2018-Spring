...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    private int binarySearch(E item, E[] data, int i, int size){
        int index = (size - i) / 2 + i;

        if(index == data.length || data[index] == null) return index;
        if(data[index] != null && data[index].equals(item)) return index;

        if(data[index].compareTo(item) > 0) {
            if(i == size || index == 0) return index;
            return binarySearch(item, data, i, index - 1);
        }
        else {
            if(i == size || index == data.length - 1) return index + 1;
            return binarySearch(item, data, index + 1, size);
        }

    }

    private boolean binarySearch(Node<E> root, E item){
        int index = binarySearch(item, root.data, 0, root.size);
        if (index != root.size && item.compareTo(root.data[index]) == 0) return true;
        if (root.child[index] == null) return false;
        return binarySearch(root.child[index], item);
    }

    public boolean binarySearch(E item){return binarySearch(root,item);}
    /*</listing>*/
