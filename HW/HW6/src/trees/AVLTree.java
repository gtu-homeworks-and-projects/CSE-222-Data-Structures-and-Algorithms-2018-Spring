...
public class AVLTree<E extends Comparable<E>>
    ...
    public AVLTree(BinaryTree<E> other){
        if(checkAVL(other)) System.out.println("Given Tree is Balanced. Checked by Constructor");
    }

    /**
     * Checks if a Binary Tree is AVL or not by checking its balance.
     * @param tree Tree to be checked
     * @param <E> Generic Method for static purposes
     * @return true or false
     */
    public static <E> boolean checkAVL(BinaryTree<E> tree){
        if(tree instanceof AVLTree) return true; // For performance gain.
        return isAVL(tree.root);
    }

    /**
     * Checks a node if its balanced or not.
     * @param node Node to be checked
     * @return result
     */
    private static <E> boolean isAVL(Node<E> node){
        if(node == null) return true;
        int balance = getHeight(node.right) - getHeight(node.left);
        return balance <= 1 && balance >= -1 && isAVL(node.left) && isAVL(node.right);
    }

    /**
     * Gets maximum height of a node till the leaf.
    ...
    private void incrementBalance(AVLNode<E> node){
        node.balance++;
        if(node.balance == AVLNode.BALANCED) increase = false;
    }

    /**
     * Method to rebalance right.
     * pre: localRoot is the root of an AVL subtree that is
     * critically right-heavy restored.
     *
     * @param localRoot Root of the AVL subtree
     *                  that needs rebalancing
     * @return a new localRoot
    ...
    /*** BOOK CODES from here ***/


    /**
     * Delete starter method. Just like the add method.
    ...
    private AVLNode<E> delete(AVLNode<E> localRoot, E item){
        ...
        if(item.compareTo(localRoot.data) == 0){
            ...
            return deleteNode(localRoot);
        } else if(item.compareTo(localRoot.data) < 0){
            localRoot.left = delete((AVLNode<E>) localRoot.left, item);
            if(decrease){
                incrementBalance(localRoot);
                if(localRoot.balance > AVLNode.RIGHT_HEAVY) return rebalanceRightLeft(localRoot);
            }
            return localRoot;
        } else{
            ...
            if(decrease){
                decrementBalance(localRoot);
                if(localRoot.balance < AVLNode.LEFT_HEAVY) return rebalanceLeftRight(localRoot);
            }
            return localRoot;
        }
    }
    ...
    /**
     * Deletes given node and fills its slot with correct node.
     * @param node The node to be removed
     * @return new node of subtree which the item removed from.
     */
    private AVLNode<E> deleteNode(AVLNode<E> node){
        if(node.left == null){
            ...
            if(node.left.right == null){
                ...
                if(((AVLNode<E>) node.left).balance < AVLNode.LEFT_HEAVY) node.left = rebalanceLeft((AVLNode<E>) node.left);
                if(decrease) incrementBalance(node);
    ...
    /**
     * Since a right rotation is needed there will be one at least. But if left child has a right child,
     * that must be rotated to the left. This method is modified version of rebalanceLeft method.
     *
    ...
    private AVLNode<E> rebalanceLeftRight(AVLNode<E> localRoot){
        ...
        if(leftChild.balance > AVLNode.BALANCED){
            AVLNode<E> leftRightChild = (AVLNode<E>) leftChild.right;
            if(leftRightChild.balance < AVLNode.BALANCED){ // This case requires left child to rotate left.
                leftChild.balance = AVLNode.LEFT_HEAVY;
                leftRightChild.balance = AVLNode.BALANCED;
                localRoot.balance = AVLNode.BALANCED;
            ...
            increase = false; // There is no increase in height but instead it will decrease cause of general tree will
            decrease = true;  // be shorter after right rotation.
            localRoot.left = rotateLeft(leftChild);
        }
    ...
    /**
     * Since a left rotation is needed there will be one at least. But if right child has a left child,
     * that must be rotated to the right. This method is modified version of rebalanceRight method.
     *
     * @param localRoot Root of the AVL subtree that needs rebalancing
     * @return a new localRoot
     */
    private AVLNode<E> rebalanceRightLeft(AVLNode<E> localRoot){
        ...
        if(rightChild.balance < AVLNode.BALANCED){
            AVLNode<E> rightLeftChild = (AVLNode<E>) rightChild.left;
            if(rightLeftChild.balance > AVLNode.BALANCED){ // This case requires right child to rotate right.
                rightChild.balance = AVLNode.RIGHT_HEAVY;
                rightLeftChild.balance = AVLNode.BALANCED;
                localRoot.balance = AVLNode.BALANCED;
            } else if(rightLeftChild.balance < AVLNode.BALANCED){
            ...
            increase = false; // There is no increase in height but instead it will decrease cause of general tree will
            decrease = true;  // be shorter after left rotation.
            localRoot.right = rotateRight(rightChild);
        }
